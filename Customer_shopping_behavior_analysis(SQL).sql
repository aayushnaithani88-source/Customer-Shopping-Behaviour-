...
-- Q2. Which customers used a discount but still spent more than average purchase amount?
 ...
 Select subscription_status, count(customer_id) as tatal_customer, round(avg(purchase_amount),2) as avg_spend, sum(purchase_amount) as total_revenue
 from customer 
 ...
 round(100 * sum(case when discount_applied = "Yes" then 1 else 0 end)/count(*),2) as dis_rate
 from customer
 group by item_purchased
...
-- Q8. What are the top 3 most puchases from each category?
with item_counts as(
Select category,
item_purchased,
